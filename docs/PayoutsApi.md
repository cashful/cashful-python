...
# **list_payouts**
> ListPayoutsResponseDto list_payouts(merchant_id, limit=limit, offset=offset, status=status)
...
with cashful.ApiClient(configuration) as api_client:
    ...
    try:
        # List Payouts
        api_response = api_instance.list_payouts(merchant_id, limit=limit, offset=offset, status=status)
